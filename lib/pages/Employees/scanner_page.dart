...
class _scannerPageState extends State<scannerPage> {
  ...
  Future<void> handleQRCodeResult(String qrCodeData) async {
    ...
    String type = parts[0].toLowerCase(); // 'veg', 'nonveg', or 'eggs'
    int count = int.tryParse(parts[1]) ?? 0;

    if (count > 0) {
      // Assuming the roll number or ID is passed to this function or available in your app context
      String roll = 'your_roll_number_here'; // Replace with actual roll number or ID

      // Convert type to corresponding token list format
      ...
      // Call the checkAndDecrementTokenData function
      List<dynamic> result = await Firestore().checkAndDecrementTokenData(token, roll);
      ...
      if (result[0] == 'already_used') {
        ...
        showAlert(context, 'Error', 'An unexpected error occurred. Please try again.');
      }
    }
  }
